...
class ServerApi(remote.Service):
    ...
    def _proximity_query(self, user_id, device_id, user_friends_ids):
            ...
            try:
                ...
                if friend_settings and friend_settings.visible:
                    ...
                    if existing_location:
                        search_users.append(existing_location)
            except IndexError:
        ...
        query_users = UserPosition.all().filter('user IN', ["test_user1", "test_user2"])
        #query_users = UserPosition.all().filter('user IN', search_users)

        if search_users:
            logging.info('Before search =====')
            for res in search_users:
...
#            pprint(search_users)
    ...
    def _format_friend_location(self, friend_id, latitude, longitude):
        ...
        logging.info("Friend: %s Lat %s Lng %s" %(friend_id, latitude, longitude))
